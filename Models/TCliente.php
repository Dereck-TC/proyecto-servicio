...
trait TCliente{
	...
	public function insertDetalle(int $idpedido, int $servicioid, float $precio, int $cantidad){
		$this->con = new Mysql();
		$query_insert  = "INSERT INTO detalle_pedido(pedidoid,servicioid,precio,cantidad) 
							  VALUES(?,?,?,?)";
	...
	public function setContacto(string $nombre, string $email, string $mensaje, int $valoracion, int $idservicio/**, string $ip, string $dispositivo, string $useragent*/){
		$this->con = new Mysql();
		...
		$query_insert  = "INSERT INTO comentario(nombre,email,mensaje,valoracion,idservicio /** ,ip,dispositivo,useragent*/) 
						  VALUES(?,?,?,?,?)";
		$arrData = array($nombre,$email,$mensaje,$valoracion,$idservicio /** ,$ip,$dispositivo,$useragent*/);
		$request_insert = $this->con->insert($query_insert,$arrData);
		return $request_insert;
	}
}
