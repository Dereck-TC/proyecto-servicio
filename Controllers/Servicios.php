	...
	class Servicios extends Controllers{
		...
		public function getServicios()
		{
			if($_SESSION['permisosMod']['r']){
				...
				// 	$idpersona = $_SESSION['userData']['idpersona'];
				// }
				$arrData = $this->model->selectServicios($idpersona);
				for ($i=0; $i < count($arrData); $i++) {
					...
					{
						$arrData[$i]['status'] = '<span class="">Activo</span>';
					}else{
						$arrData[$i]['status'] = '<span class="">Inactivo</span>';
					}
		...
		public function valoracion(){
			...
			$request_servicio = $this->model->updateValoracion($idServicio,$valoracion);
			if($request_servicio > 0){
				$arrResponse = array('status' => true, 'idservicio' => $request_servicio, 'msg' => 'Gracias por tu valoración.');
			}else{
				$arrResponse = array('status' => false, 'msg' => 'No se pudo guardar los datos.');
			}
